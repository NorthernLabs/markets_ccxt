...
var alephx$1 = require('./abstract/alephx.js');
...
class alephx extends alephx$1 {
    describe() {
        return this.deepExtend(super.describe(), {
            ...
            'exceptions': {
                'exact': {},
                'broad': {
                    'Wallet not allowed': errors.AuthenticationError,
                    'Invalid signature': errors.AuthenticationError,
                    'Unauthorized': errors.PermissionDenied,
                    'Order is not cancellable': errors.BadRequest,
                    'Asset is not supported': errors.BadRequest,
    ...
    async createOrder(symbol, type, side, amount, price = undefined, params = {}) {
        ...
        if (errorResponse !== undefined) {
            ...
            throw new errors.ExchangeError(errorReason + '' + errorMessage);
        }
    ...
    async cancelOrder(id, symbol = undefined, params = {}) {
        ...
        if (errorResponse !== undefined) {
            ...
            if (errorReason === 'Not Found') {
                throw new errors.OrderNotFound(this.id + ' cancelOrder() error ' + errorReason);
            }
            else if (errorReason === 'Bad Request') {
                throw new errors.BadRequest(this.id + ' cancelOrder() error ' + errorReason + ' ' + errorMessage);
            }
            else {
                throw new errors.ExchangeError(errorReason + '' + errorMessage);
            }
    ...
    async fetchOrder(id, symbol = undefined, params = {}) {
        ...
        if (errorResponse !== undefined) {
            const errorReason = this.safeString(errorResponse, 'reason');
            if (errorReason === 'Not Found') {
                throw new errors.OrderNotFound(this.id + ' fetchOrder() error ' + errorReason);
            }
        }
        return this.parseOrder(response);
    }
